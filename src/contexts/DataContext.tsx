...
interface DataContextType {
  organizations: Organization[]; events: Event[]; archivedEvents: Event[]; participants: Participant[]; users: User[]; activityLog: ActivityLog[]; currentRole: Role; currentUser: User; selectedOrganizationId: string; setSelectedOrganizationId: (id: string) => void; selectedEventId: string; setSelectedEventId: (id: string) => void;
  ...
  createEvent: (e: EventMutationInput) => Promise<MutationResult>; updateEvent: (eventId: string, data: EventMutationInput) => Promise<MutationResult>; deleteEvent: (eventId: string) => Promise<MutationResult>; addUser: (u: UserCreateInput) => Promise<MutationResult>; updateUser: (userId: string, data: UserUpdateInput) => Promise<MutationResult>; createOrganization: (data: { name: string; event_limit: number; admin_user_id?: string }) => Promise<MutationResult>; updateOrganization: (organizationId: string, data: OrganizationUpdateInput) => Promise<MutationResult>; updateOrganizationEventLimit: (organizationId: string, eventLimit: number) => Promise<MutationResult>; deleteOrganization: (organizationId: string) => Promise<MutationResult>; removeUser: (id: string) => Promise<MutationResult>; triggerUserPasswordReset: (id: string) => Promise<MutationResult>; changeRole: (userId: string, role: Role) => Promise<MutationResult>; assignScannerEvents: (userId: string, eventIds: string[]) => Promise<MutationResult>;
  ...
  visibleEvents: Event[]; canAccessEvent: (eventId: string) => boolean; canViewEvent: (eventId: string) => boolean; isLoading: boolean;
}
...
type ApiEvent = Event;
interface BootstrapResponse { data: { organizations: Organization[]; events: ApiEvent[]; archivedEvents?: ApiEvent[]; users: ApiUser[]; participants: ApiParticipant[]; activityLog: ActivityLog[]; }; }
...
export function DataProvider({ children }: { children: ReactNode }) {
  ...
  const [organizations, setOrganizations] = useState<Organization[]>([]); const [events, setEvents] = useState<Event[]>([]); const [archivedEvents, setArchivedEvents] = useState<Event[]>([]); const [participants, setParticipants] = useState<Participant[]>([]); const [users, setUsers] = useState<User[]>([]); const [activityLog, setActivityLog] = useState<ActivityLog[]>([]); const [selectedOrganizationId, setSelectedOrganizationIdState] = useState<string>(''); const [selectedEventId, setSelectedEventIdState] = useState<string>(''); const [isLoading, setIsLoading] = useState(true); const [nowTimestamp, setNowTimestamp] = useState(() => Date.now());
  const resetState = useCallback(() => { setOrganizations([]); setEvents([]); setArchivedEvents([]); setParticipants([]); setUsers([]); setActivityLog([]); setSelectedOrganizationIdState(''); setSelectedEventIdState(''); }, []);
  const persistSelectedOrganizationId = useCallback((organizationId: string, userId?: string | null) => { if (!userId) return; try { const key = getSelectedOrganizationStorageKey(userId); if (organizationId) { localStorage.setItem(key, organizationId); return; } localStorage.removeItem(key); } catch { /* Ignore unavailable localStorage. */ } }, []);
  ...
  const persistSelectedEventId = useCallback((eventId: string, userId?: string | null) => { if (!userId) return; try { const key = getSelectedEventStorageKey(userId); if (eventId) { localStorage.setItem(key, eventId); return; } localStorage.removeItem(key); } catch { /* Ignore unavailable localStorage. */ } }, []);
  ...
  const syncStoredAuthUser = useCallback((updater: (user: User) => User) => { try { const raw = sessionStorage.getItem('auth_user'); if (!raw) return; const parsed = JSON.parse(raw) as User; sessionStorage.setItem('auth_user', JSON.stringify(updater(parsed))); } catch { /* Ignore unavailable sessionStorage. */ } }, []);
  ...
  const updateUserInApi = useCallback(async (userId: string, data: UserUpdateInput): Promise<User> => {
    const response = await fetch(`${API_BASE_URL}/users/${userId}`, { method: 'PATCH', headers: getAuthHeaders(true), body: JSON.stringify(data) }); if (!response.ok) { const payload = await response.json().catch(() => ({})) as { error?: string }; throw new Error(payload.error ?? `API user update failed: ${response.status}`); }
    const payload = await response.json() as { data?: ApiUser }; if (!payload.data) throw new Error('API user update returned empty payload'); return mapApiUserToUi(payload.data);
  }, [getAuthHeaders]);
  const triggerUserPasswordResetInApi = useCallback(async (userId: string): Promise<void> => {
    const response = await fetch(`${API_BASE_URL}/users/${userId}/password-reset`, { method: 'POST', headers: getAuthHeaders() }); if (!response.ok) { const payload = await response.json().catch(() => ({})) as { error?: string }; throw new Error(payload.error ?? `API user password reset failed: ${response.status}`); }
  }, [getAuthHeaders]);
  ...
  const deleteEvent = useCallback(async (eventId: string): Promise<MutationResult> => {
    ...
    try {
      ...
      if (existingEvent) {
        setArchivedEvents(previous => [{ ...existingEvent, archived_at: new Date().toISOString() }, ...previous]);
      }
      if (selectedEventId === eventId) {
        setSelectedEventId('');
      }
      if (existingEvent) addLog(`Zarchiwizowano wydarzenie: ${existingEvent.name}`);
      await loadBootstrap().catch(() => undefined);
      return { ok: true };
    } catch (error) {
      return { ok: false, error: error instanceof Error ? error.message : 'Nie udało się zarchiwizować wydarzenia' };
    }
  }, [addLog, deleteEventInApi, events, loadBootstrap, selectedEventId, setSelectedEventId]);
  ...
  const updateUser = useCallback(async (userId: string, userData: UserUpdateInput): Promise<MutationResult> => {
    try { const updatedUser = await updateUserInApi(userId, userData); setUsers(previous => previous.map(user => user.id === userId ? updatedUser : user)); syncStoredAuthUser(user => user.id === userId ? updatedUser : user); return { ok: true }; } catch (error) { return { ok: false, error: error instanceof Error ? error.message : 'Nie udało się zaktualizować użytkownika' }; }
  }, [syncStoredAuthUser, updateUserInApi]);
  ...
  const updateOrganizationEventLimit = useCallback(async (organizationId: string, eventLimit: number): Promise<MutationResult> => {
    ...
    try { const updatedOrganization = await updateOrganizationEventLimitInApi(organizationId, eventLimit); setOrganizations(previous => previous.map(organization => organization.id === organizationId ? updatedOrganization : organization)); await loadBootstrap().catch(() => undefined); return { ok: true }; } catch (error) { return { ok: false, error: error instanceof Error ? error.message : 'Nie udało się zaktualizować limitu wydarzeń' }; }
  }, [events, loadBootstrap, updateOrganizationEventLimitInApi]);
  ...
  const triggerUserPasswordReset = useCallback(async (id: string): Promise<MutationResult> => {
    const existingUser = users.find(user => user.id === id);
    try { await triggerUserPasswordResetInApi(id); if (existingUser) addLog(`Wysłano reset hasła użytkownikowi: ${existingUser.name}`); return { ok: true }; } catch (error) { return { ok: false, error: error instanceof Error ? error.message : 'Nie udało się wysłać resetu hasła' }; }
  }, [addLog, triggerUserPasswordResetInApi, users]);
  ...
  const scanParticipantQr = useCallback(async (qrCode: string) => {
    const response = await fetch(`${API_BASE_URL}/participants/scan`, { method: 'POST', headers: getAuthHeaders(true), body: JSON.stringify({ qr_code: qrCode }) });
