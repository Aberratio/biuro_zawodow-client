...
const refreshTimeFormatter = new Intl.DateTimeFormat("pl-PL", {
  hour: "2-digit",
  minute: "2-digit",
});
...
// Etykiety masowej wysyłki są długie — na wąskich ekranach muszą się zawijać
// zamiast wychodzić poza kartę (Button ma w bazie whitespace-nowrap i stałą wysokość).
const BULK_ACTION_BUTTON_CLASS =
  "h-auto min-h-11 w-full whitespace-normal px-4 py-2.5 text-center leading-snug sm:min-h-10";
...
const formatDeliveryTimestamp = (value: string | null | undefined): string => {
  ...
  return Number.isNaN(parsed.getTime())
    ? value
    : qrActionDateFormatter.format(parsed);
};

const isQrSendingActivity = (action: string) => {
  const normalizedAction = action.toLowerCase();
  return (
    normalizedAction.includes("kod qr") ||
    normalizedAction.includes("kody qr") ||
    normalizedAction.includes("kodu qr")
  );
};
...
const formatQrActivityAction = (log: ActivityLog) => {
  ...
  if (
    normalizedAction === "nie udało się wysłać kodu qr" ||
    normalizedAction.startsWith("nie udało się wysłać kodu qr:")
  ) {
  ...
  if (
    normalizedAction === "nie udało się ponownie wysłać kodu qr" ||
    normalizedAction.startsWith("nie udało się ponownie wysłać kodu qr:")
  ) {
    return log.participant_name
  ...
  return log.participant_name &&
    normalizedAction.includes("kod qr") &&
    !normalizedAction.includes(log.participant_name.toLowerCase())
...
const compareParticipantsByStableListOrder = (
  ...
  first.id.localeCompare(second.id, "pl", {
    numeric: true,
    sensitivity: "base",
  });
...
export default function EmailSending() {
  ...
  const [sendingParticipantId, setSendingParticipantId] = useState<
    string | null
  >(null);
  const [pendingAction, setPendingAction] = useState<PendingEmailAction | null>(
    null,
  );
  const [deliveryReport, setDeliveryReport] =
    useState<QrEmailDeliveryReport | null>(null);
  ...
  const isOfficeOpenNow = selectedEvent
    ? isEventOfficeOpen(selectedEvent)
    : false;
  const canSendBulkQrEmails = selectedEvent
    ? isEventCurrentOrUpcoming(selectedEvent)
    : false;
  ...
  const recentQrActions = useMemo(
    () =>
      activityLog
        .filter(
          (log) =>
            log.event_id === activeEventId && isQrSendingActivity(log.action),
        )
        .slice(0, 5),
    [activeEventId, activityLog],
  );
  const isConfirmingAction =
    sendingAll || resendingAll || sendingParticipantId !== null;
  const isSendingQrEmails =
    sendingAll || resendingAll || sendingParticipantId !== null;
      ...
      try {
        ...
        if (!silent) {
          toast({
            ...
            description:
              "Pokazujemy lokalny stan wysyłki. Spróbuj odświeżyć za chwilę.",
            variant: "destructive",
          });
        }
      } finally {
  ...
  const deliveryDataAvailable =
    deliveryReport !== null &&
    deliveryReport.mailer_available &&
  ...
  const participantDeliveryRows = useMemo<ParticipantDeliveryRow[]>(
    () =>
      eventParticipants.map((participant) => {
        ...
        const displayStatus: QrDeliveryDisplayStatus | null =
  ...
  const filteredDeliveryRows = useMemo(() => {
    ...
    return participantDeliveryRows.filter(({ participant, displayStatus }) => {
      if (deliveryStatusFilter === "local_not_sent") {
        if (!(displayStatus === null && participant.email_status !== "sent"))
          return false;
      } else if (
        deliveryStatusFilter !== "all" &&
        displayStatus !== deliveryStatusFilter
      ) {
        return false;
      }
      if (!search) return true;
      return [participant.name, participant.email, participant.bib_number].some(
        (value) =>
          String(value ?? "")
            .toLowerCase()
            .includes(search),
  ...
  const handleSendAll = async (
    ...
    if (resendAll && isOfficeOpenNow) {
      ...
      toast({
        ...
        description:
          "W godzinach działania biura można wysyłać tylko brakujące kody QR.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      const result = await sendEventQrEmails(
        activeEventId,
        resendAll,
        paymentScope,
      );
      if (!result.ok) {
        toast({
          title: "Nie udało się wysłać kodów QR",
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="py-3">
          <div className="flex items-start gap-2">
            ...
            <div className="space-y-1 text-xs text-muted-foreground">
              {!canSendBulkQrEmails ? (
                <p>
                  Po zamknięciu biura zawodów masowa wysyłka kodów QR jest
                  niedostępna.
                </p>
              ) : hasNoSentEmails ? (
                <p>
                  "Wyślij kody QR do wszystkich uczestników" wyśle wiadomość
                  email z kodami qr do wszystkich uczestników na aktualnej
                  liście.
                </p>
              ) : (
                <>
                  {!isOfficeOpenNow && (
                    <p>
                      "Wyślij ponownie kody QR do wszystkich uczestników" wymusi
                      ponowną wysyłkę dla całego wydarzenia.
                    </p>
                  )}
                  {hasPartialDelivery && (
                    <p>
                      "Wyślij brakujące kody QR" wyśle wiadomości tylko do
                      uczestników, którzy nie dostali jeszcze maila z kodem QR.
                    </p>
                  )}
                  {isOfficeOpenNow && (
                    <p>
                      W godzinach działania biura ponowna wysyłka do wszystkich
                      jest zablokowana, ale nadal możesz wysłać brakujące kody
                      QR.
                    </p>
                  )}
                </>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Status wysyłki</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-bold tabular-nums">
              {sent}/{eventParticipants.length}
            </div>
            <p className="mt-1 text-sm text-muted-foreground">
              uczestników ma już mail z QR
            </p>
            {selectedEvent && (
              <p className="mt-2 text-xs text-muted-foreground">
                Biuro: {formatEventOfficeSchedule(selectedEvent)}
              </p>
            )}
            <div className="mt-3 h-2 rounded-full bg-muted">
              <div
                ...
                style={{
                  width: `${eventParticipants.length ? (sent / eventParticipants.length) * 100 : 0}%`,
                }}
              />
            </div>
            <div
                  ...
                  {deliverySummary && (
                    <>
                      <Badge variant="default" className="text-[10px]">
                        Wysłane: {deliverySummary.sent}
                      </Badge>
                      <Badge variant="secondary" className="text-[10px]">
                        W kolejce: {deliverySummary.queued}
                      </Badge>
                      {deliverySummary.bounced > 0 && (
                        <Badge variant="destructive" className="text-[10px]">
                          Odbite: {deliverySummary.bounced}
                        </Badge>
                      )}
                      {deliverySummary.failed > 0 && (
                        <Badge variant="destructive" className="text-[10px]">
                          Błędy: {deliverySummary.failed}
                        </Badge>
                      )}
                      {deliverySummary.unknown > 0 && (
                        <Badge variant="outline" className="text-[10px]">
                          Nieznane: {deliverySummary.unknown}
                        </Badge>
                      )}
                      {deliverySummary.no_data > 0 && (
                        <Badge variant="outline" className="text-[10px]">
                          Brak danych: {deliverySummary.no_data}
                        </Badge>
                      )}
                    </>
                  )}
            ...
            >
              <div className="overflow-hidden">
                ...
                >
                  ...
                  <span>
                    Serwer pocztowy potwierdził wysłanie wszystkich maili z
                    kodami QR.
                  </span>
                </div>
              </div>
            ...
            <div className="mt-4 grid gap-2">
              ...
              {!canSendBulkQrEmails && (
                <p className="text-sm text-muted-foreground">
                  To wydarzenie ma już zamknięte biuro zawodów, więc masowa
                  wysyłka została wyłączona.
                </p>
              )}
              {canSendBulkQrEmails &&
                isOfficeOpenNow &&
                ...
                !hasNoSentEmails && (
                  <p className="text-sm text-muted-foreground">
                    W godzinach działania biura nie można ponownie wysłać kodów
                    QR do wszystkich uczestników.
                  </p>
                )}
            </div>
          </CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Historia ostatnich operacji wysyłki kodów QR (5 ostatnich, resztę
              możesz eksportować w zakładce ze szczególami tego wydarzenia).
            </CardDescription>
          </CardHeader>
                ...
                >
                  <div className="space-y-1.5">
                    <p className="text-sm font-medium leading-5">
                      {formatQrActivityAction(log)}
                    </p>
                    <div className="flex flex-wrap gap-x-3 gap-y-1 text-xs text-muted-foreground">
                      <span>
                        {qrActionDateFormatter.format(new Date(log.timestamp))}
                      </span>
                      <span>
                        {log.user_name
                          ? `Wysłał(a): ${log.user_name}`
                          : "Operator nieznany"}
                      </span>
                    </div>
                  </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Lista uczestników</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="mb-4 flex flex-col gap-2 sm:flex-row">
            ...
            >
              <SelectTrigger
                className="sm:w-64"
                aria-label="Filtr statusu dostarczenia"
              >
                <SelectValue placeholder="Wszystkie statusy" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">Wszystkie statusy</SelectItem>
                <SelectItem value="local_not_sent">
          ...
          {/* Tylko tabela przewija się poziomo — filtry zostają w paddingu karty. */}
          <div className="-mx-6 overflow-x-auto px-6">
            <Table>
              ...
              <TableBody>
                ...
                {filteredDeliveryRows.map(
                  ({ participant, delivery, displayStatus }) => {
                    const statusDefinition =
                      displayStatus !== null
                        ? getQrDeliveryStatusDefinition(displayStatus)
                        : null;
                    const showError =
                      delivery?.last_error &&
                        ...
                        <TableCell>
                          <div className="flex min-h-9 flex-col justify-center gap-1">
                            <div className="flex flex-wrap items-center gap-1">
                              {statusDefinition ? (
                                <Badge
                                  variant={statusDefinition.badgeVariant}
                              ...
                              {delivery && (
                                <Badge
                                  ...
                                  title={
                                    delivery.is_batch
                                      ? `Wysyłka masowa (batch: ${delivery.batch_id ?? ""})`
                                      : "Wysyłka pojedyncza"
                                  }
                                >
            ...
            >
              {pendingAction?.kind === "send-one" ? (
                <>
                  ...
                  <span className="font-medium text-foreground">
                    {pendingAction.participantEmail}
                  </span>
                  .
                </>
              ) : pendingAction?.kind === "resend-all" ? (
