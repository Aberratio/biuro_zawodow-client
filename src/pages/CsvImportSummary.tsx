...
function getSourceRow(
  sourceRows: Record<string, string>[] | undefined,
  rowNumber: number
...
function resolveEmailColumn(
  ...
  return (
    headers.find((header) =>
      issues.some((issue) => (issue.row?.[header] ?? "").includes("@"))
    ) ?? ""
  );
}

function buildImportSuccessDescription(
  createdCount: number,
  mode: ImportSummaryState["mode"]
): string {
...
function findMappingForHeader(
  mappings: ParticipantFieldMapping[],
  header: string
): ParticipantFieldMapping | undefined {
  return mappings.find(
    (mapping) => mapping.source_column_name === header && mapping.is_active
  );
}
...
function getIssueFieldErrors(
  ...
  if (emailColumn) {
    ...
    if (!email) {
      ...
      errors[emailColumn] =
        `Adres e-mail może mieć maksymalnie ${PARTICIPANT_EMAIL_MAX_LENGTH} znaków.`;
...
function getIssueReasonFieldHints(
  ...
  issue.reasons.forEach((reason) => {
    ...
    const matchingField = fieldNames.find((field) =>
      field.matches.some(
        (match) =>
          quotedValues.includes(match) || normalizedReason.includes(match)
      )
    );
...
function buildRowValidationErrors(
  ...
  return issues.reduce<RowValidationErrors>((accumulator, issue) => {
    accumulator[issue.row_number] = getIssueFieldErrors(
      issue,
      headers,
      mappings,
      emailColumn
        ...
        >
          <TableHeader className="sticky top-0 z-10 bg-background">
            <TableRow>
              ...
              {emailColumn && (
                <TableHead className="min-w-[15rem]">E-mail</TableHead>
              )}
              <TableHead className="min-w-[18rem]">Problem</TableHead>
              <TableHead className="min-w-[18rem]">Podgląd</TableHead>
              {isEditable && (
                <TableHead className="w-[11rem] text-right">Akcja</TableHead>
              )}
            </TableRow>
          </TableHeader>
          <TableBody>
            {issues.map((issue) => {
              ...
              const errorCount =
                Object.values(rowErrors).filter(Boolean).length;
                ...
                <TableRow key={`${title}-${issue.row_number}`}>
                    ...
                    <div className="space-y-1">
                      {errorCount > 0 && (
                        <Badge variant="destructive">
                          {errorCount} pól do poprawy
                        </Badge>
                      )}
                      {issue.reasons.slice(0, 2).map((reason, index) => (
                        <p
                          key={`${issue.row_number}-${index}`}
                      ...
                      {issue.reasons.length > 2 && (
                        <p className="text-xs text-muted-foreground">
                          + {issue.reasons.length - 2} więcej w edycji
                        </p>
                      )}
                    </div>
                  </TableCell>
                  ...
                  {isEditable && (
                    <TableCell className="align-top text-right">
                      ...
                      >
                        {savingRowNumbers[issue.row_number] ? (
...
function ParticipantIssueEditorDialog({
  ...
  const reasonHints = useMemo(
    () =>
      issue
        ? getIssueReasonFieldHints(issue, headers, mappings, emailColumn)
        : {},
    ...
    <Dialog open={Boolean(issue)} onOpenChange={onOpenChange}>
      {issue && (
        <DialogContent className="max-h-[min(42rem,calc(100vh-2rem))] max-w-3xl overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              Edycja uczestnika z wiersza {issue.row_number}
            </DialogTitle>
            <DialogDescription>
            ...
            {issue.reasons.length > 0 && (
              <div className="rounded-md border border-amber-400/45 bg-amber-500/10 px-3 py-2">
                <p className="text-sm font-medium">
                  Błędy wskazane przez import
                </p>
                <ul className="mt-2 space-y-1 text-sm text-muted-foreground">
                  {issue.reasons.map((reason, index) => (
                    <li key={`${issue.row_number}-reason-${index}`}>
                      {reason}
                    </li>
                  ))}
                </ul>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {headers.map((header) => {
                ...
                const mapping =
                  header === emailColumn
                    ? undefined
                    : findMappingForHeader(mappings, header);
                  ...
                  >
                    ...
                    {mapping && mapping.field_role !== "email" ? (
                      <ParticipantFieldValueInput
                        ...
                        onChange={(value) =>
                          onFieldChange(issue.row_number, header, value)
                        }
...
export default function CsvImportSummary() {
  ...
  const [editableInvalidIssues, setEditableInvalidIssues] = useState<
    ImportRowIssue[]
  >([]);
  const [retryingImport, setRetryingImport] = useState(false);
  const [savingRowNumbers, setSavingRowNumbers] = useState<
  ...
  const editingIssue =
    editingRowNumber === null
      ? null
      : (editableInvalidIssues.find(
          (issue) => issue.row_number === editingRowNumber
        ) ?? null);
  const canRetryEditedRows =
  ...
  const changedEmailCount = useMemo(
    () =>
      editableInvalidIssues.filter((issue) => {
        const originalIssue = invalidIssues.find(
          (item) => item.row_number === issue.row_number
        );
        return (
          (issue.row?.[emailColumn] ?? "") !==
          (originalIssue?.row?.[emailColumn] ?? "")
        );
      }).length,
  ...
  const currentCreatedCount =
    (summary?.created_count ?? 0) + importedFixedCount;
  ...
  const applyRetryResultToRows = (
    ...
    const remainingIssues = attemptedIssues
      .map((issue, index) => {
        ...
        return {
          ...issue,
          reasons:
            failedDetail.reasons.length > 0
              ? failedDetail.reasons
              : issue.reasons,
    ...
    setEditableInvalidIssues((previous) =>
      [
        ...previous.filter(
          (issue) =>
            !attemptedIssues.some(
              (attempted) => attempted.row_number === issue.row_number
            )
        ),
        ...remainingIssues,
    ...
    if (successfulOriginalRows.size > 0) {
      setImportedFixedCount(
        (previous) => previous + successfulOriginalRows.size
      );
    }
  };
  ...
  const handleSaveIssue = async (issue: ImportRowIssue) => {
    ...
    const validationErrors = getIssueFieldErrors(
      issue,
      invalidHeaders,
      summaryMappings,
      emailColumn
    ...
    try {
      setSavingRowNumbers((previous) => ({
        ...previous,
        [issue.row_number]: true,
      }));
      const result = await runParticipantImport(
        eventId,
        buildCsvContent(invalidHeaders, [issue])
      );
        ...
        toast({
          ...
          description:
            result.duplicate_count > 0
              ? "Ten rekord nadal wygląda jak duplikat."
              : "Sprawdź dane w wierszu i spróbuj ponownie.",
          variant: "destructive",
        });
      }
    } catch (error) {
      toast({
        ...
        description:
          error instanceof Error
            ? error.message
            : "Spróbuj ponownie albo pobierz CSV do poprawy.",
        variant: "destructive",
      });
    } finally {
      setSavingRowNumbers((previous) => ({
        ...previous,
        [issue.row_number]: false,
      }));
    }
  };
  ...
  const handleRetryEditedRows = async () => {
    ...
    const nextErrors = editableInvalidIssues.reduce<RowValidationErrors>(
      (accumulator, issue) => {
        const errors = getIssueFieldErrors(
          issue,
          invalidHeaders,
          summaryMappings,
          emailColumn
        ...
        if (Object.values(errors).some(Boolean))
          accumulator[issue.row_number] = errors;
        return accumulator;
      },
      {}
    );
    ...
    if (
      Object.values(nextErrors).some((rowErrors) =>
        Object.values(rowErrors).some(Boolean)
      )
    ) {
      toast({
        title: "Popraw błędne pola przed importem",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description:
          result.invalid_count + result.duplicate_count > 0
  ...
  const handleReturnToValidationSettings = () => {
    const restoredAnalysis = {
      ...(state.analysis ?? {
        ...
        email_candidates: state.emailColumn
          ? [
              {
                column: state.emailColumn,
                matched_count: state.sourceRows?.length ?? 0,
              },
            ]
          : [],
    ...
    return (
      <div className="mx-auto max-w-6xl space-y-6">
        <Button
          variant="ghost"
          size="sm"
          onClick={() => navigate(buildEventImportPath(eventId))}
          className="w-fit rounded-full px-1 text-[0.98rem] font-medium text-[hsl(var(--button-highlight))] hover:bg-transparent hover:text-[hsl(var(--button-highlight))]"
        >
          <ArrowLeft className="mr-1 h-4 w-4" /> Wróć do importu
        </Button>
        <PageHeader
          title="Podsumowanie importu CSV"
          description={`Wydarzenie: ${event.name}`}
        />
        <Alert>
          <FileWarning className="h-4 w-4" />
          <AlertTitle>Brak danych podsumowania</AlertTitle>
          <AlertDescription>
            Podsumowanie jest dostępne bezpośrednio po imporcie pliku CSV.
          </AlertDescription>
        </Alert>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
        ...
        <PageHeader
          ...
          description={
            <>
              ...
              {state.fileName && (
                <p className="mt-1 text-xs">Plik: {state.fileName}</p>
              )}
            </>
          }
          actions={
      ...
      <div className="grid gap-3 md:grid-cols-4">
        <StatCard
          label="Zaimportowano"
          value={currentCreatedCount}
          tone="success"
        />
        <StatCard
          label="Nie zaimportowano"
          value={unimportedCount}
          tone={unimportedCount > 0 ? "warning" : "default"}
        />
        <StatCard
          label="Duplikaty"
          value={summary.duplicate_count}
          tone={summary.duplicate_count > 0 ? "warning" : "default"}
        />
        <StatCard
          label="Błędne wiersze"
          value={currentInvalidCount}
          tone={currentInvalidCount > 0 ? "danger" : "default"}
        />
      </div>
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        ...
        {unimportedCount > 0 ? (
          <Alert className="border-amber-400/50 bg-amber-500/10">
            ...
            <AlertDescription>
              {currentInvalidCount > 0
                ? "Błędne dane można poprawić w tabeli poniżej i od razu dopisać zawodnika do bazy. "
                : ""}
              {summary.duplicate_count > 0
                ? "Duplikaty pominięto, bo pasują do uczestników już zapisanych w wydarzeniu."
                : ""}
            </AlertDescription>
          </Alert>
        ) : (
          <Alert>
            ...
            <AlertDescription>
              Nie wykryto duplikatów ani błędnych rekordów.
            </AlertDescription>
          </Alert>
        )}
      </div>
      ...
      {editableInvalidIssues.length > 0 && !emailColumn && (
        <Alert className="border-amber-400/50 bg-amber-500/10">
          ...
          <AlertDescription>
            Możesz nadal pobrać CSV do poprawy. Edycja w tabeli wymaga
            rozpoznanej kolumny z adresem e-mail.
          </AlertDescription>
        </Alert>
      )}
          ...
          <CardContent className="flex flex-col gap-3 py-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Możesz poprawić dowolną kolumnę w tabeli poniżej, w tym e-mail (
                <span className="font-medium text-foreground">
                  {emailColumn}
                </span>
                ).
                {changedEmailCount > 0
                  ? ` Zmieniono ${changedEmailCount} adresów e-mail.`
                  : " Wprowadź poprawki w tabeli poniżej."}
              </p>
            </div>
            <Button
              onClick={handleRetryEditedRows}
      ...
      <ParticipantIssueEditorDialog
        ...
        fieldErrors={
          editingIssue
            ? (rowValidationErrors[editingIssue.row_number] ?? {})
            : {}
        }
        saving={
          editingIssue
            ? Boolean(savingRowNumbers[editingIssue.row_number])
            : false
        }
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="space-y-2">
              <span className="block">
                Ta operacja usunie wszystkich uczestników tego wydarzenia razem
                z mapowaniem CSV, bazą importu i logami zmian uczestników.
              </span>
              {resetRequiresQrConfirm && (
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <Button onClick={() => navigate(buildEventPath(eventId))}>
