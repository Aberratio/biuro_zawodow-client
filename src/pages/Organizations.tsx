      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground sm:text-sm">
            Przegląd organizacji. Wejdź do wybranej organizacji, aby zobaczyć wydarzenia, zespoły i limity.
          </p>
        </div>
            ...
            <Card key={org.id} className="overflow-hidden border-border/70 shadow-sm">
              <CardHeader className="border-b bg-muted/30">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">Podsumowanie organizacji i dostępnych zasobów.</p>
                  </div>
              ...
              <CardContent className="space-y-4 p-5">
                <div className="flex flex-wrap gap-2">
                  <Badge variant="secondary">{orgEvents.length}/{org.event_limit} wydarzeń</Badge>
                ...
                <div className="rounded-xl border bg-background px-4 py-3 text-sm text-muted-foreground">
                  Kliknij poniżej, aby wejść do środka organizacji i zarządzać szczegółami.
                </div>
                <Button className="h-11 w-full justify-between sm:h-10" variant="outline" onClick={() => navigate(`/organizations/${org.id}`)}>
                  Wejdź do organizacji
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-[calc(100vw-2rem)] sm:max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div><Label>Limit wydarzeń</Label><Input type="number" min="0" value={form.event_limit} onChange={e => setForm(prev => ({ ...prev, event_limit: e.target.value }))} /></div>
          ...
          <DialogFooter>
            <Button className="w-full sm:w-auto" onClick={handleCreate} disabled={!form.name || !form.event_limit || (currentRole === 'superadmin' && !form.admin_user_id) || isSubmitting}>Utwórz organizację</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
